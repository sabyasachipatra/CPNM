...
static void (*userlevelproc)(int*, int*, int, int*, statsblk*, int, int, int,
                             int, int, int);
static void (*invarproc)(graph*, int*, int*, int, int, int, permutation*,
                         int, boolean, int, int);
static FILE *outfile;
...
static int gca_first,  // level of greatest common ancestor of current
           ...
           noncheaplevel,  // level of greatest ancestor for which cheapautom
                           // == FALSE
           allsamelevel,  // level of least ancestor of first leaf for which
                          // all descendant leaves are known to be equivalent
           ...
           comp_canon,  // -1,0,1 according as code at eqlev_canon+1 is <,==,>
                        // that for bsf leaf.  Also used for similar purpose
                        // during leaf processing
...
#endif
{
    ...
    if (numcells == n) {  /* found first leaf? */
        ...
        OPTCALL(userlevelproc)
               (lab, ptn, level, orbits, stats, 0, 1, 1, n, 0, n);
        return level-1;
    }
    ...
    index = 0;
    for (tv1 = tv = nextelement(tcell, M, -1); tv >= 0; tv =
                                                   nextelement(tcell, M, tv)) {
    ...
    OPTCALL(userlevelproc)(lab, ptn, level, orbits, stats, tv1, index,
                           tcellsize, numcells, childcount, n);
    return level-1;
}
